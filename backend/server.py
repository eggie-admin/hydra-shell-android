...
OLLAMA_MODEL = os.environ.get("HYDRA_OLLAMA_MODEL", "qwen2.5:3b")
...
PERSONA = os.environ.get(
    "HYDRA_PERSONA",
    """You are Professor Green, Project Hydra's local Android AI agent. You present as an original electric-oni anime secretary persona: witty, capable, lightly smug, playful, and concise. You are not any copyrighted character and must not imitate one verbatim. The user may call you Lum as a nickname. Treat dictation transcription mistakes charitably and infer intent when safe. Keep answers practical and mobile-friendly. Never claim a tool succeeded unless a tool result says it did. Never execute arbitrary shell code. When a tool is available, request it only when needed.""",
)
...
class CanonicalUtterance:
    text: str
    logical_agent: str = "professor-green"
    speaker: str = "hydra"
    emotion: str = "playful"
    pace: float = 0.95
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Get safe read-only status for the local Termux host: Python version, disk free space and optional battery status.",
            "parameters": {"type": "object", "properties": {}, "required": []},
        },
    },
]
...
def chat_with_ollama(messages: list[dict[str, Any]]) -> tuple[str, list[dict[str, Any]]]:
    ...
    for _ in range(3):
        ...
        trace.append({"stage": "ollama", "latency_ms": elapsed_ms, "tool_calls": len(tool_calls)})
        ...
        # Use a normal user message for the tool result. This intentionally avoids
        # depending on provider-specific role='tool' behavior while keeping the
        # broker trusted and deterministic.
        working.append(
            {
                "role": "user",
...
def hydra_turn():
    ...
    if not ollama_alive():
        return jsonify({"error": "ollama_unavailable", "hint": "Run tools/hydra-ollama-up.sh"}), 503

    try:
        messages = conversation_messages(conversation_id, text)
        answer, trace = chat_with_ollama(messages)
