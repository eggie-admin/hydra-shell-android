...
const chat=document.querySelector('#chat'),form=document.querySelector('#form'),promptEl=document.querySelector('#prompt'),send=document.querySelector('#send'),model=document.querySelector('#model'),mic=document.querySelector('#mic');
...
form.onsubmit=e=>{e.preventDefault();void submitMessage()};
const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(SR){const rec=new SR();rec.lang='en-US';rec.interimResults=false;rec.continuous=false;mic.onclick=()=>{if(busy)return;try{rec.start()}catch(err){setMicState('idle')}};rec.onstart=()=>setMicState('listening');rec.onresult=e=>{if(busy)return;const result=e.results[e.results.length-1];if(!result||!result.isFinal)return;promptEl.value=result[0].transcript.trim();if(!promptEl.value)return;setMicState('sending');void submitMessage()};rec.onerror=()=>setMicState('idle');rec.onend=()=>{if(!busy)setMicState('idle')}}else mic.disabled=true;
</script></body></html>'''
