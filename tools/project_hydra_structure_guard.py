...
def main() -> None:
    ...
    eq(project.get("parent_os"), "LumH OS", "parent OS")
    eq(project.get("branch_authority"), "main", "branch authority")
    ...
    eq(project.get("platforms", {}).get("samsung_android_apk"), "project/hydra/samsung/android/apk/app.reference.json", "Samsung APK path")
    ...
    eq(apk.get("source", {}).get("repository"), "eggie-admin/vue-headless-cms", "APK source repo")
    eq(apk.get("source", {}).get("branch"), "samsung-sm-x400-build-candidate", "APK source branch")
    eq(apk.get("requirements", {}).get("installable_apk_required_for_green"), True, "APK artifact gate")
    eq(apk.get("requirements", {}).get("loopback_only_local_services"), True, "APK loopback policy")
    ...
    for local_ref in (
        ...
        ROOT / hf["doctrine"],
        ROOT / apk["local_compatibility_reference"],
    ):
